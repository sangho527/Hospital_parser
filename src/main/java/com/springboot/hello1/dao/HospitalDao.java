...
public class HospitalDao {
    ...
    RowMapper<Hospital> rowMapper = (rs, rowNum) -> {
        Hospital hospital = new Hospital(); //16줄
        ...
        hospital.setLisenseDate(rs.getTimestamp("license_date").toLocalDateTime());
        hospital.setTotalAreaSize(rs.getFloat("total_area_size"));
        return hospital;
    };

    public Hospital findById(int id) {
        return this.jdbcTemplate.queryForObject("select * from nation_wide_hospitals1 where id = ?", rowMapper, id);
    }

    public int getCount() {
        String sql = "select count(id) from nation_wide_hospitals1;";
        return this.jdbcTemplate.queryForObject(sql, Integer.class);
    }

    public void deleteAll() {
        this.jdbcTemplate.update("delete from nation_wide_hospitals1");
    }

    public void add(Hospital hospital) {
        String sql = "INSERT INTO `sh-db`.`nation_wide_hospitals1` (`id`, `open_service_name`, `open_local_government_code`, `management_number`, `license_date`, `business_status`, `business_status_code`, `phone`, `full_address`, `road_name_address`, `hospital_name`, `business_type_name`, `healthcare_provider_count`, `patient_room_count`, `total_number_of_beds`, `total_area_size`)" +
                " VALUES (?,?,?," +
                " ?,?,?," +
                " ?,?,?," +
                " ?,?,?," +
